...
### Changed
- The previous `StaticRoutingProtocol` class is split into two classes: `StaticRoutingProtocol` and `ForwardingProtocol`. This can be viewed as the separation of control plane and the data plane for the entanglement routing module. The control plane do the routing, the data plane do the forwarding. The Routing Protocols writes the `forwarding_table`, while the Forwarding Protocol  reads the `forwarding_table`.
