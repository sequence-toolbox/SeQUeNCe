...
class DensityState(State):
    ...
    def __init__(
        self,
        state: OneDimensionInput | TwoDimensionInput,
        keys: list[int],
        truncation: int = 1,
    ):
        ...
        if state.ndim == 1:
            ...
            raise ValueError(
                "Density matrix state must be a 1D state vector or a 2D matrix."
            )
        if state.shape[0] != state.shape[1]:
            raise ValueError("Density matrix must be square.")

        # check formatting
        assert abs(np.trace(np.array(state)) - 1) < 0.01, (
            "density matrix trace must be 1"
        )
