...
class KetState(State):
    ...
    def __init__(
        self, amplitudes: OneDimensionInput, keys: list[int], truncation: int = 1
    ):
        ...
        # check formatting
        assert all([abs(a) <= 1 + EPSILON for a in amplitudes]), (
            "Illegal value with abs > 1 in ket vector"
        )
        assert math.isclose(sum([abs(a) ** 2 for a in amplitudes]), 1), (
