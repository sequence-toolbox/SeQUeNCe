...
class QuantumManagerStabilizer(QuantumManager):
    ...
    def __init__(self, seed: int | None = None, **kwargs):
        ...
        """
        super().__init__()
        self.base_seed: int | None = (
            seed  # Base seed controls deterministic per-state/per-operation seed derivation.
        )
        self._seed_counter: int = (
            0  # Monotonic counter used with base_seed to produce unique child seeds.
        )
        self.noise_rng: np.random.Generator = np.random.default_rng(seed)
        self.one_qubit_gate_fid: float = self._validate(
            kwargs.get("one_qubit_gate_fid", 1.0)
        )
        self.two_qubit_gate_fid: float = self._validate(
            kwargs.get("two_qubit_gate_fid", 1.0)
        )
        self.measurement_fid: float = self._validate(kwargs.get("measurement_fid", 1.0))
        self.initialization_fid: float = self._validate(
            kwargs.get("initialization_fid", 1.0)
        )
        ...
        self.pauli_1q_weights: tuple[float, ...] = self._normalize_pauli_weights(
            kwargs.get("pauli_1q_weights", (1.0, 1.0, 1.0)), 3, "pauli_1q_weights"
        )
        self.pauli_2q_weights: tuple[float, ...] = self._initialize_pauli_2q_weights(
            kwargs.get("pauli_2q_weights")
        )
        self.last_idle_time_ps_by_key: dict[
            int, int
        ] = {}  # Last active simulation time per key.
    ...
    def _initialize_pauli_2q_weights(
        self, raw_pauli_2q_weights: tuple | None = None
    ) -> tuple[float, ...]:
        ...
        if not isinstance(raw_pauli_2q_weights, tuple):
            raise TypeError(
                "pauli_2q_weights must be a tuple of 15 numeric weights or None"
            )
        return self._normalize_pauli_weights(
            raw_pauli_2q_weights, 15, "pauli_2q_weights"
        )

    def new(
        self, state: StabilizerState | Tableau | TableauSimulator | list | None = None
    ) -> int:
    ...
    def set(
        self,
        keys: list[int],
        state: StabilizerState | Tableau | TableauSimulator | list,
    ):
    ...
    def run_circuit(
        self, circuit: Circuit, keys: list[int], inject_gate_error: bool = False
    ) -> dict[int, int]:
        ...
        for instruction in circuit:
            ...
            if name not in supported_names:
                raise ValueError(
                    f"Unsupported stim instruction for stabilizer manager: {name}"
                )
            ...
            if name in {"M", "MX", "MY"}:
                ...
                raise ValueError(
                    "Stabilizer manager only supports terminal measurements."
                )
            instruction_payloads.append((name, targets))

        # Prepare validated inputs, merged/shared topology, and key index mapping.
        state_obj, key_to_local = self._prepare_circuit(
            len(keys), measured_qubits, keys
        )
        ...
        if not inject_gate_error:
            ...
            for name, targets in instruction_payloads:
                if name in {"M", "MX", "MY"}:
                    continue
                for elementary_targets in self._iter_elementary_gate_targets(
                    name, targets
                ):
                    local_targets = [
                        key_to_local[keys[target]] for target in elementary_targets
                    ]
        ...
        if (
            len(measured_qubits) == 0
        ):  # No measurement: commit a fresh shared state object for all keys.
            committed_state = StabilizerState(
                state=simulator, keys=list(state_obj.keys)
            )
            ...
            for name, targets in instruction_payloads:
                ...
                for target in targets:
                    ...
                    if (
                        inject_gate_error
                        and self.measurement_fid < 1.0
                        and self.noise_rng.random() > self.measurement_fid
                    ):
                        reported_bit ^= 1
                    ...
                    if physical_bit == 1:
                        collapsed.x(0)
                    self.states[measured_key] = StabilizerState(
                        state=collapsed, keys=[measured_key]
                    )

            # Physically drop measured qubits so simulator indices stay compact for remaining keys.
            simulator, remaining_keys = self._drop_keys_from_stabilizer_simulator(
                simulator, state_obj.keys, measured_keys
            )
    ...
    def _iter_elementary_gate_targets(
        self, gate_name: str, targets: list[int]
    ) -> list[list[int]]:
        ...
        if gate_name in {"CX", "CZ", "SWAP"}:
            if len(targets) % 2 != 0:
                raise ValueError(
                    f"Expected even target count for {gate_name}, got {len(targets)}."
                )
    ...
    def get_circuit_duration(self, circuit: Circuit) -> int:
        ...
        for instruction in circuit:
            ...
            if any(
                not getattr(target, "is_qubit_target", False) for target in targets_raw
            ):
                raise RuntimeError(
                    f"Unsupported non-qubit target in duration estimate: {name}"
                )
    ...
    def apply_idling_decoherence(
        self, keys: list[int], now_ps: int, t1_sec: float, t2_sec: float
    ):
        ...
        if not (0 < t2_sec <= 2 * t1_sec):
            raise ValueError(
                f"Invalid T1/T2 values: T1={t1_sec}, T2={t2_sec}. Must satisfy 0 < T2 <= 2*T1."
            )
        ...
        for key in keys:
            ...
            if idle_sec > 0:  # Only proceed if there is a positive idle interval
                ...
                py = px
                pz = (
                    1.0 + np.exp(-idle_sec / t1_sec) - 2.0 * np.exp(-idle_sec / t2_sec)
                ) / 4.0
                local = key_to_local[key]
                noise_circuit = Circuit()
                if self.idle_error_channel == "depolarize":
                    p_idle = float(px + py + pz)
                    noise_circuit.append(
                        "PAULI_CHANNEL_1",
                        [local],
                        [p_idle / 3.0, p_idle / 3.0, p_idle / 3.0],
                    )
                elif self.idle_error_channel == "pauli":
                    noise_circuit.append(
                        "PAULI_CHANNEL_1", [local], [float(px), float(py), float(pz)]
                    )
                else:
                    raise ValueError(
                        "idle_error_channel must be 'depolarize' or 'pauli'."
                    )
    ...
    def _apply_initialization_fidelity(
        self, simulator: TableauSimulator, targets: list[int]
    ):
    ...
    @staticmethod
    def _normalize_pauli_weights(
        weights: tuple[float, ...], expected_length: int, name: str
    ) -> tuple[float, ...]:
    ...
    @staticmethod
    def _derive_default_pauli_2q_weights(
        pauli_1q_weights: tuple[float, ...], single_only_fraction: float = 0.8
    ) -> tuple[float, ...]:
        ...
        return (
            ...
            correlated_fraction * z_weight * z_weight,
        )  # ZZ

    def _sample_pauli_channel_branch(
        self, channel_name: str, probs: list[float], targets: list[int], source: str
    ) -> str:
        ...
        if total <= 0.0:
            ...
            sampled_index = int(
                self.noise_rng.choice(len(branch_labels), p=normalized_probs)
            )
            sampled_branch = branch_labels[sampled_index]

        log.logger.info(
            f"pauli_channel_sample source={source} channel={channel_name} targets={targets} "
            f"branch={sampled_branch} inserted_error={int(sampled_branch != identity_label)}"
        )
        return sampled_branch

    @staticmethod
    def _apply_sampled_pauli_branch(
        simulator: TableauSimulator, targets: list[int], sampled_branch: str
    ):
    ...
    def _initialize_stabilizer_state(
        self,
        initializer: StabilizerState | Tableau | TableauSimulator | list,
        keys: list[int],
    ) -> StabilizerState:
        ...
        if isinstance(initializer, TableauSimulator):
            simulator = (
                initializer.copy() if hasattr(initializer, "copy") else initializer
            )
        ...
        if isinstance(initializer, Tableau):
            ...
            tableau = Tableau.from_state_vector(
                np.asarray(initializer), endian="little"
            )
        ...
        if len(tableau) != len(keys):
            raise ValueError(
                f"Initializer tableau has {len(tableau)} qubits but {len(keys)} keys were supplied."
            )
    ...
    def _apply_gate_error(
        self, simulator: TableauSimulator, gate_name: str, targets: list[int]
    ):
        ...
        if name in {"H", "X", "Y", "Z", "S", "S_DAG"}:
            ...
            if p_error > 0:
                if self.gate_error_channel == "depolarize":
                    ...
                    raise ValueError(
                        "gate_error_channel must be 'depolarize' or 'pauli'."
                    )
                sampled_branch = self._sample_pauli_channel_branch(
                    "PAULI_CHANNEL_1", probs, targets, f"gate:{name}"
                )
                if sampled_branch != "I":
        ...
        if name in {"CX", "CZ", "SWAP"}:
            ...
            if p_error > 0:
                ...
                sampled_branch = self._sample_pauli_channel_branch(
                    "PAULI_CHANNEL_2", probs, targets, f"gate:{name}"
                )
    ...
    def _prepare_circuit(
        self, num_qubits: int, measured_qubits: list[int], keys: list[int]
        ...
        if len(keys) != num_qubits:
            raise ValueError(
                f"circuit width ({num_qubits}) must equal len(keys) ({len(keys)})."
            )
        if not keys:
            return None, {}
        ...
        if len(set(measured_qubits)) != len(measured_qubits):
            raise ValueError(
                f"Duplicate measured qubit indices are not allowed: {measured_qubits}"
            )
        ...
        for key in keys:
            ...
            if not isinstance(qstate, StabilizerState):
                raise ValueError(
                    f"Expected StabilizerState for key {key}, got {type(qstate)}"
                )
            if qstate.state.num_qubits != len(qstate.keys):
                raise RuntimeError(
                    f"Tableau/key mismatch for state keys: {qstate.keys}"
                )
    ...
    @staticmethod
    def _drop_keys_from_stabilizer_simulator(
        simulator: TableauSimulator, state_keys: list[int], drop_keys: list[int]
    ) -> tuple[TableauSimulator, list[int]]:
