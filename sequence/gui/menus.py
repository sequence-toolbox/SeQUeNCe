...
OPTIONS_NODE = [
    {'label': 'Quantum Router', 'value': 'QuantumRouter'},
    {'label': 'BSM Node', 'value': 'BSMNode'},
    {'label': 'QKD Node', 'value': 'QKDNode'},
]
...
OPTIONS_TEMPLATE = [
    ...
    {'label': 'BSM Node', 'value': 'BSMNode'},
    {'label': 'Photon Source', 'value': 'PhotonSource'},
    {'label': 'QKD Node', 'value': 'QKDNode'},
]
...
def getInputField(value_in: str, label: str, input_id: str, out_type: str, style_in=None, place=None):
    return dbc.Row(
        [
            dbc.Col(dbc.Label(label), width=4),
            dbc.Col(dbc.Input(id=input_id, value=value_in, className=out_type, placeholder=place), width=8),
        ],
        style=style_in,
    )
...
def getDropdownField(value_in: str, all_vals: 'list[str]', label: str, input_id: str, out_type: str, style_in=None):

    return dbc.Row(
        [
            dbc.Col(dbc.Label(label), width=4),
            dbc.Col(dcc.Dropdown(id=input_id, options=all_vals, value=value_in, className=out_type), width=8),
        ],
        style=style_in,
    )
...
def getLogo(filename, width, link=None):
    ...
    image = html.Img(src=f'data:image/png;base64,{encoded_image.decode()}', width=width, style={'border-radius': '20%'})
    if link is None:
        return image
    else:
...
def getSelectedNodeMenu(values, templates):
    out = []
    out.append(getInputField(values['name'], 'Name:', 'selected_name', 'name'))
    out.append(getDropdownField(values['type'], OPTIONS_NODE, 'Node Type:', 'selected_node_type', 'type'))
    out.append(
        getDropdownField(
            values['template'], makeDropdownOptions(templates), 'Template:', 'selected_template', 'template'
        )
    )
...
def getSelectedEdgeMenu(values, nodes, link_types):
    return dbc.Form(
        [
            getDropdownField(values['source'], makeDropdownOptions(nodes), 'Source:', 'selected_source', 'source'),
            getDropdownField(values['target'], makeDropdownOptions(nodes), 'Target:', 'selected_target', 'target'),
            getDropdownField(
                values['link_type'], makeDropdownOptions(link_types), 'Link Type:', 'selected_link_type', 'link_type'
            ),
            getInputField(values['attenuation'], 'Attenuation (dB/m):', 'selected_attenuation', 'attenuation'),
            getInputField(values['distance'], 'Distance (m):', 'selected_distance', 'distance'),
        ]
    )
...
classic_edge = [
    getInputField('', 'Distance (m):', 'distance_input', ''),
    getInputField('', 'Attenuation (dB/m):', 'attenuation_input', ''),
]
...
router_template = [
    ...
    dbc.Input(id='mem_size', className='memo_size', placeholder='Memory Array Size', type='number'),
    dbc.Label('Memory Type'),
    dcc.Dropdown(id='mem_type', className='mem_type', value='', options=[]),
]
...
detector_template = [
    ...
    dbc.Label('Efficiency'),
    dbc.Input(id='detector_efficiency_in', className='efficiency', placeholder='0.8'),
    dbc.Label('Count Rate'),
    dbc.Input(id='count_rate_in', className='count_rate', placeholder='5.7e3'),
    dbc.Label('Resolution'),
    dbc.Input(id='resolution_in', className='resolution', placeholder='1e2'),
]

bsm_template = [
    dbc.Label('Detector 1 Type'),
    dcc.Dropdown(id='detec_type_1', className='detector_type', value='', options=[]),
    dbc.Label('Detector 2 Type'),
    dcc.Dropdown(id='detec_type_2', className='detector_type', value='', options=[]),
]

qkd_template = [
    dbc.Label('Photon Encoding'),
    dcc.Dropdown(id='encoding_in', className='encoding', value='', options=['polarization', 'time_bin']),
    dbc.Label('Protocol Stack Size'),
    dcc.Dropdown(id='stack_size_in', className='stack_size', value='', options=[1, 2, 3, 4, 5]),
]
...
add_node_form = html.Div(
    dbc.Form(
        [
            html.H3('Add Node'),
            getInputField('', 'Name:', 'node_to_add_name', '', place='Enter Node ID'),
            getDropdownField('QuantumRouter', OPTIONS_NODE, 'Type:', 'type_menu', ''),
            getDropdownField('', [], 'Template:', 'add_template_menu', '', style_in={'margin-bottom': '10px'}),
            html.Div([dbc.Button('Add Node', color='primary', id='add_node')], className='d-grid'),
...
add_edge = html.Div(
    [
        html.H3('Add Edge'),
        getDropdownField('', [], 'From:', 'from_node', ''),
        getDropdownField('', [], 'To:', 'to_node', ''),
        getDropdownField(
            '',
            [
...
delete_menu = html.Div(
    dbc.Form(
        [
            ...
            dbc.Row(
                [
                    html.P('Select an element and press the button to remove it'),
                    html.Div(
                        [
                            dbc.Button(
                                'Delete',
...
make_new_template = html.Div(
    dbc.Form(
        [
            html.H3('Template'),
            getInputField('', 'ID:', 'template_name', '', place='Enter ID'),
            getDropdownField('QuantumRouter', OPTIONS_TEMPLATE, 'Type:', 'template_type_menu', ''),
            dbc.Row(id='template_properties'),
            html.Div(
                [
                    dbc.Button(
                        'Save',
            ...
            [
                ...
                dbc.ButtonGroup(
                    [
                        ...
                        dbc.Button('Edges', id='toggle_edges', color='primary', style={'padding': '5px 65px'}),
                    ],
                    style={
                        'margin-bottom': '10px',
                    },
...
def TDM_menu(tdm_data, tdm_columns):
    ...
    tdm_columns.insert(0, {'id': 'To', 'type': 'text', 'name': 'To'})
...
selection_menu = html.Div(
    [
        ...
        html.Div(
            [dbc.Button('Submit', id='submit_edit', color='primary', style={'margin-top': '10px'})], className='d-grid'
        ),
    ],
    id=tab_ids[7],
    style=MENU_STYLE,
...
results_menu = dbc.Form(
    [
        ...
        dbc.Card(
            ...
            style={'minHeight': '50vh', 'maxHeight': '65vh', 'overflowY': 'auto', 'overflowX': 'auto'},
        ),
    ]
)
    ...
    [
        ...
        dbc.Row(
            [
                ...
                dbc.Col(dbc.Input(id='sim_name', placeholder='Ex: Test_1'), width=10),
            ],
            className='g-0',
        ),
        dbc.Row(
            [
                dbc.Col(dbc.Label('Time'), width=2),
                dbc.Col(dbc.Input(id='sim_time_in', placeholder='Enter simulation time'), width=8),
                dbc.Col(
                    getTimeUnits('sim'),
                    width=2,
                ),
        ...
        dcc.Interval(id='running', interval=1000, n_intervals=0, disabled=True),
        html.Pre(id='runtime'),
        html.Pre(id='simtime'),
        results_menu,
    ],
        ...
        html.A(
            dbc.Row(
                [
                    ...
                    dbc.Col(dbc.NavbarBrand('SeQUeNCe', className='ml-2', style={'font-size': '50px'})),
                ],
                align='center',
                className='g-0',
            ),
