    ...
    def __init__(self, sim_time: int, time_scale: int, logging: str, sim_name, config):
        ...
        for node in nodes:
            ...
            if node_type == "QKDNode":
                node_in = QKDNode(node_name, self.timeline, **node_temp)
            elif node_type == "QuantumRouter":
                mem = node_temp.pop("mem_type")
                mem_config = self.sim_templates[mem].copy()
                node_in = QuantumRouter(node_name, self.timeline, **node_temp)
        ...
        for i in range(len(table)):
            for j in range(len(table[i])):
                ...
                self.topology.add_classical_channel(
                    labels[i], labels[j], **cchannel_params
                )
        ...
        for node in self.topology.get_nodes_by_type("QuantumRouter"):
            ...
            for dst, next_node in table.items():
                node.network_manager.routing_protocol.update_forwarding_rule(
                    dst, next_node
                )

    def init_logging(self):
        set_logger(
            "sim_logging", self.timeline, DIRECTORY + "/" + self.sim_name + "_log.txt"
        )
