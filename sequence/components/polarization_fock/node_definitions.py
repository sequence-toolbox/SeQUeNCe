...
class PolarizationReceiverNode(Node):
    ...
    def receive_qubit(self, src: str, qubit) -> None:
        rotated_photon = self.components[self.first_component_name].get(qubit)

        if self.temp_received_photon == None:
            self.temp_received_photon = rotated_photon
        else:
            self.detectors.get(rotated_photon, port=self.port_list[0])
            self.detectors.get(self.temp_received_photon, port=self.port_list[1])
            self.temp_received_photon = None
    ...
    def receive_proxy_qubit(self, src: str, qubit) -> None:
        ...
        if self.temp_received_photon == None:
            ...
            self.detectors.get(rotated_photon, port=self.port_list[1])
            self.detectors.get(self.temp_received_photon, port=self.port_list[0])
            self.temp_received_photon = None
    ...
    def get_data(self):
        ...
        #     print(i)
        for idler_angle_detection_probs,idler_angle_detection_var in zip(self.idler_rotation_detection_probs,self.idler_rotation_detection_vars):
            coincidence_probs = []
            coincidence_std = []
            ...
            for detection_probs,detection_squared_probs in zip(idler_angle_detection_probs,idler_angle_detection_var):
