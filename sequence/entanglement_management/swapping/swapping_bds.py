...
class EntanglementSwappingA_BDS(EntanglementSwappingA):
    ...
    def start(self) -> None:
        ...
        log.logger.info(
            f"{self.owner.name} middle protocol start with ends {self.left_protocol_name}, {self.right_protocol_name}"
        )
        ...
        if self.owner.get_generator().random() < self.success_probability():
            log.logger.info(f"{self.owner.name} Swapping succeeded! Protocol: {self}")
            self.is_success = True
            expire_time = min(
                self.left_memo.get_expire_time(), self.right_memo.get_expire_time()
            )
            ...
            left_remote_memory: Memory = self.owner.timeline.get_entity_by_name(
                self.left_remote_memo
            )
            right_remote_memory: Memory = self.owner.timeline.get_entity_by_name(
            ...
            log.logger.info("swapping failed!")
            msg_l = EntanglementSwappingMessage(
    ...
    def swapping_res(self) -> list[float]:
        ...
        right_state = self.owner.timeline.quantum_manager.get(
            self.right_memo.qstate_key
        )

        left_elem_1, left_elem_2, left_elem_3, left_elem_4 = (
            left_state.state
        )  # BDS diagonal elements of left pair
        right_elem_1, right_elem_2, right_elem_3, right_elem_4 = (
            right_state.state
        )  # BDS diagonal elements of right pair
        ...
        c_I = (
            left_elem_1 * right_elem_1
            + left_elem_2 * right_elem_2
            + left_elem_3 * right_elem_3
            + left_elem_4 * right_elem_4
        )
        c_X = (
            left_elem_1 * right_elem_2
            + left_elem_2 * right_elem_1
            + left_elem_3 * right_elem_4
            + left_elem_4 * right_elem_3
        )
        c_Y = (
            left_elem_1 * right_elem_4
            + left_elem_4 * right_elem_1
            + left_elem_2 * right_elem_3
            + left_elem_3 * right_elem_2
        )
        c_Z = (
            left_elem_1 * right_elem_3
            + left_elem_3 * right_elem_1
            + left_elem_2 * right_elem_4
            + left_elem_4 * right_elem_2
        )
        ...
        log.logger.info(
            f"{self.owner.name} before swapping, f = {left_state.state[0]:.6f}, {right_state.state[0]:.6f}; after swapping, f = {bds_elems[0]:.6f}"
        )
...
class EntanglementSwappingB_BDS(EntanglementSwappingB):
    ...
    def received_message(self, src: str, msg: EntanglementSwappingMessage) -> None:
        ...
        log.logger.debug(
            self.owner.name
            + " protocol received_message from node {}, fidelity={:.6f}".format(
                src, msg.fidelity
            )
        )
        ...
        if msg.fidelity > 0 and self.owner.timeline.now() < msg.expire_time:
            ...
            remote_memory: Memory = self.owner.timeline.get_entity_by_name(
                msg.remote_memo
            )
