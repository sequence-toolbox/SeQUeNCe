...
class EntanglementSwappingA_BDS(EntanglementSwappingA):
    ...
    def swapping_res(self) -> list[float]:
        ...
        new_elem_1 = (
            gate_fid * (meas_fid**2 * c_I + meas_fid * (1 - meas_fid) * (c_X + c_Z) + (1 - meas_fid) ** 2 * c_Y)
            + (1 - gate_fid) / 4
        )
        new_elem_2 = (
            gate_fid * (meas_fid**2 * c_X + meas_fid * (1 - meas_fid) * (c_I + c_Y) + (1 - meas_fid) ** 2 * c_Z)
            + (1 - gate_fid) / 4
        )
        new_elem_3 = (
            gate_fid * (meas_fid**2 * c_Z + meas_fid * (1 - meas_fid) * (c_I + c_Y) + (1 - meas_fid) ** 2 * c_X)
            + (1 - gate_fid) / 4
        )
        new_elem_4 = (
            gate_fid * (meas_fid**2 * c_Y + meas_fid * (1 - meas_fid) * (c_X + c_Z) + (1 - meas_fid) ** 2 * c_I)
            + (1 - gate_fid) / 4
        )
...
class EntanglementSwappingB_BDS(EntanglementSwappingB):
    ...
    def received_message(self, src: str, msg: EntanglementSwappingMessage) -> None:
        ...
        log.logger.debug(
            self.owner.name + ' protocol received_message from node {}, fidelity={:.6f}'.format(src, msg.fidelity)
        )
