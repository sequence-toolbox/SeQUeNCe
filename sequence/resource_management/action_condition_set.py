...
def eg_rule_action1(memories_info: list[MemoryInfo], args: Arguments) -> tuple[EntanglementGenerationA, list, list, list]:
...
def eg_rule_action2(memories_info: list["MemoryInfo"], args: Arguments) -> tuple[
    ...
    memory = memories[0]
    protocol = EntanglementGenerationA.create(None, "EGA." + memory.name, mid, path[index + 1], memory)
...
def ep_rule_action2(memories_info: list["MemoryInfo"], args: Arguments) -> tuple[
    ...
    protocol = BBPSSWProtocol.create(TempNode, name, memories[0], None)
    return protocol, [None], [None], [None]


def ep_req_func1(protocols, args: Arguments) -> BBPSSWProtocol | None:
