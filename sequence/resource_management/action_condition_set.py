...
def ep_rule_action1(memories_info: list[MemoryInfo], _args: Arguments) -> ActionReturn:
    ...
    req_args = [
        {
            "remote0": memories_info[0].remote_memo,
...
def ep_rule_condition1(
    ...
    if purification_mode == "until_target":
        if (
            memory_info.index in memory_indices  # this memory (kept)
            ...
            for info in memory_manager:
                if (
                    info != memory_info
                    and info.index in memory_indices  # another memory (meas)
...
def es_rule_conditionA(
    ...
    if (
        ...
        for memory_info2 in memory_manager:
            if (
                memory_info2.state in ["ENTANGLED", "PURIFIED"]
...
def es_rule_conditionB1(
    ...
    target_remote = args[
        "target_remote"
    ]  # A - B - C. For A: B is the remote node, C is the target remote
    fidelity = args["fidelity"]
    if (
        memory_info.state in ["ENTANGLED", "PURIFIED"]
        and memory_info.index in memory_indices
