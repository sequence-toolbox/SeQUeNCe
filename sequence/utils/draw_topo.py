...
parser.add_argument(
    "-d", "--directory", type=str, default="tmp", help="directory to save the figure"
)
parser.add_argument(
    "-f", "--filename", type=str, default="topology", help="filename of the figure"
)
...
if node_type == RouterNetTopo.BSM_NODE or node_type == RouterNetTopo.QUANTUM_ROUTER:
    ...
    raise Exception(
        "Unknown node type '{}' in config file {}".format(node_type, args.config_file)
    )
...
if draw_middle:
    ...
    for bsm, nodes in bsm_to_node.items():
        assert len(nodes) == 2, (
            f"{bsm} connects to {len(nodes)} number of nodes (should be 2)"
        )
