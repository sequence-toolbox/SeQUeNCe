...
def generate_config(
    ...
    if "router_template" not in templates or "bsm_template" not in templates:
        raise ValueError(
            "Template must contain 'router_template' and 'bsm_template' keys."
        )
    ...
    nodes: list[dict] = generate_nodes(
        ...
        memory_size,
        "router_template",
        measurement_fidelity=meas_fid,
        gate_fidelity=gate_fid,
    )
    graph_to_name = {
        graph_node: router_names[i] for i, graph_node in enumerate(g.nodes)
    }
