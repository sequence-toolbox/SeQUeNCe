...
default_template = {
    'router_template': {
        'MemoryArray': {'frequency': 200000000.0, 'coherence_time': 2, 'efficiency': 1, 'fidelity': 0.9}
    },
    'bsm_template': {
        'encoding_type': 'single_heralded',
        'SingleHeraldedBSM': {
            'detectors': [
                {'efficiency': 1, 'dark_count': 0, 'time_resolution': 6, 'count_rate': 100000000000.0},
                {'efficiency': 1, 'dark_count': 0, 'time_resolution': 6, 'count_rate': 100000000000.0},
            ]
        },
    },
...
def generate_classical(router_names: list, cc_delay: float) -> list:
    ...
    for node1 in router_names:
        for node2 in router_names:
            if node1 == node2:
                continue
            cchannels.append({Topology.SRC: node1, Topology.DST: node2, Topology.DELAY: int(cc_delay * MILLISECOND)})
    return cchannels


def generate_nodes(
    router_names: list, memo_size: int, template: str = '', gate_fidelity: float = 1, measurement_fidelity: float = 1
) -> list:
    """
...
def generate_config(
    ...
    nodes: list[dict] = generate_nodes(
        router_names, memory_size, 'router_template', measurement_fidelity=meas_fid, gate_fidelity=gate_fid
    )
    ...
    for i, (left, right, data) in enumerate(g.edges(data=True)):
        ...
        bsm_nodes.append(
            {
                Topology.NAME: bsm_name,
                Topology.TYPE: Topology.BSM_NODE,
                Topology.SEED: i,
                Topology.TEMPLATE: 'bsm_template',
            }
        )
        ...
        qlinks.append(
            {
                Topology.SRC: left_name,
                Topology.DST: bsm_name,
                Topology.DISTANCE: to_bsm_dist,
                Topology.ATTENUATION: qc_attn,
            }
        )
        qlinks.append(
            {
                Topology.SRC: right_name,
                Topology.DST: bsm_name,
                Topology.DISTANCE: to_bsm_dist,
                Topology.ATTENUATION: qc_attn,
            }
        ...
        clinks.append(
            {
                Topology.SRC: bsm_name,
                Topology.DST: left_name,
                Topology.DISTANCE: to_bsm_dist,
                Topology.DELAY: int(cc_delay * MILLISECOND),
            }
        )
