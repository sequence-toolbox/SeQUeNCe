...
def build_caveman(cliques: int, size: int, length: float=10.0) -> nx.Graph:
    """
    ...
    nx.set_edge_attributes(G, length, name='length')  # type: ignore[arg-type]
    return G

def build_grid(size_x: int, size_y: int, length: float=10.0) -> nx.Graph:
    """
    Create a grid graph
    Args:
...
def build_star(outer_nodes: int, length: float=10.0) -> nx.Graph:
    """
    Create a star graph.
    Args:
...
def build_linear(nodes: int, length: float=10.0) -> nx.Graph:
    """
...
def build_mesh(size_x: int, size_y: int, length: float=10.0) -> nx.Graph:
    """
    ...
    Returns: NetworkX Graph
    """
    G = build_grid(size_x, size_y, length=length)

    for (x, y) in G.nodes:
        for dx, dy in [(1,1), (1,-1), (-1, 1), (-1, -1)]: # Diagonals from any node with max Deg = 4
            if (x+dx, y+dy) in G.nodes:
                v = (x+dx, y+dy)
                G.add_edge((x, y), v, length=dist((x, y), v) * length)
    return G

def build_ring(nodes: int, length: float=10.0) -> nx.Graph:
    """
...
def build_waxman(nodes, area_size: float=10.0, alpha=0.1, beta=0.4, L=None, seed=None) -> nx.Graph:
    """
...
def build_tree(branching_factor: int, nodes: int, length: float=10.0) -> nx.Graph:
    """
...
def build_autonomous_system(nodes: int, length: float=10.0, seed=None) -> nx.Graph:
    """
    Create a AS graph.
    Args:
        nodes: Number of nodes
...
def build_bcube(k: int, n: int, length: float=10.0) -> nx.Graph:
    """
    ...
    for l, s in switches:  # noqa: E741
        for i in range(n):  # There are n-ports on each switch
            ...
            G.add_edge((l, s), server, length=length)
    return G


def build_k_n(k: int, n: int, length: float=10.0) -> nx.Graph:
    """
    ...
    for l in range(n - 1):  # noqa: E741
        for w in product(range(k), repeat=n - 1):
            for w_l in range(k):
                w_prime = list(w)
                w_prime[l] = w_l
                G.add_edge((w, l), (tuple(w_prime), l + 1), length=length)

    # Create Switch -> Node edges
    for w in product(range(k), repeat=n - 1):
        for p_last in range(k):
            node = w + (p_last,)
            G.add_edge((w, n - 1), node, length=length)
