...
class TeleportApp(RequestApp):
    ...
    def start(
        self, responder: str, start_t: int, end_t: int, memory_size: int, fidelity: float, data_memory_index: int
    ):
        ...
        teleport_protocol = TeleportProtocol(
            self.node, alice=True, data_memory_index=data_memory_index, remote_node_name=responder
        )
        ...
        if msg.msg_type is TeleportMsgType.MEASUREMENT_RESULT:  # Bob receives measurement result from Alice
            for teleport_protocol in self.teleport_protocols:  # find the correct teleport protocol on Bob's side
                if (
                    ...
                    and msg.bob_comm_memory_name == teleport_protocol.bob_comm_memory_name
                ):
