...
from ..entanglement_management.teleportation import (
    TeleportMsgType,
    TeleportProtocol,
    TeleportMessage,
)
...
class TeleportApp(RequestApp):
    ...
    def __init__(self, node: DQCNode):
        ...
        node.teleport_app = (
            self  # register ourselves so incoming TeleportMessage lands here:
        )
        self.results = []  # where we’ll collect Bob’s teleported state
        self.teleport_protocols: list[
            TeleportProtocol
        ] = []  # a list of teleport protocol instances
    ...
    def start(
        ...
        """
        log.logger.debug(
            f"{self.name}: start() → responder={responder}, data_memory_index={data_memory_index}"
        )
        ...
        teleport_protocol = TeleportProtocol(
            self.node,
            alice=True,
    ...
    def get_memory(self, info: MemoryInfo):
        ...
        """
        log.logger.debug(
            f"{self.name}: get_memory, name={info.memory.name}, state={info.state}"
        )
        ...
        if info.index in self.memo_to_reservation:
            if info.state == "ENTANGLED":
                for teleport_protocol in self.teleport_protocols:
                    ...
                    if (
                        this_node == teleport_protocol.owner.name
                        and remote_node == teleport_protocol.remote_node_name
                    ):
                        # this node is Alice
                    ...
                    teleport_protocol = TeleportProtocol(
                        self.node, alice=False, remote_node_name=info.remote_node
                    )
    ...
    def received_message(self, src: str, msg: TeleportMessage):
        ...
        if (
            msg.msg_type is TeleportMsgType.MEASUREMENT_RESULT
        ):  # Bob receives measurement result from Alice
            for teleport_protocol in (
                self.teleport_protocols
            ):  # find the correct teleport protocol on Bob's side
                if (
                    src == teleport_protocol.remote_node_name
                    and msg.bob_comm_memory_name
                    == teleport_protocol.bob_comm_memory_name
                ):
                    teleport_protocol.received_message(src, msg)
                    self.node.resource_manager.expire_rules_by_reservation(
                        msg.reservation
                    )  # early release of resources
                    self.node.resource_manager.update(
                        None, teleport_protocol.bob_comm_memory, MemoryInfo.RAW
                    )  # release the bob comm memory
                    teleport_protocol.bob_acknowledge_complete(msg.reservation)
                    self.teleport_protocols.remove(
                        teleport_protocol
                    )  # remove the protocol instance, it's lifecycle is complete
                    break
            else:
                log.logger.warning(
                    f"{self.name}: received_message: no matching teleport protocol for msg={msg} from {src}"
                )

        elif (
            msg.msg_type is TeleportMsgType.ACK
        ):  # Alice receives acknowledgment from Bob
            for teleport_protocol in (
                self.teleport_protocols
            ):  # find the correct teleport protocol on Alice's side
                if (
                    src == teleport_protocol.remote_node_name
                    and msg.bob_comm_memory_name
                    == teleport_protocol.bob_comm_memory_name
                    ...
                    self.node.resource_manager.update(
                        None, teleport_protocol.alice_comm_memory, MemoryInfo.RAW
                    )  # release the alice comm memory
                    self.teleport_protocols.remove(
                        teleport_protocol
                    )  # remove the protocol instance, it's lifecycle is complete
                    break
    ...
    def teleport_complete(self, comm_key: int):
        ...
        self.results.append(
            (self.node.timeline.now(), psi)
        )  # append result (timestamp, state)
