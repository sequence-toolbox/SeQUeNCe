...
class RequestApp:
    ...
    def start(
        self, responder: str, start_t: int, end_t: int, memo_size: int, fidelity: float
    ):
    ...
    def get_memory(self, info: MemoryInfo) -> None:
        ...
        if info.index in self.memo_to_reservation:
            ...
            if (
                ...
                log.logger.info(
                    f"Successfully generated entanglement. Counter is at {self.memory_counter}."
                )
                self.node.resource_manager.update(None, info.memory, "RAW")
    ...
    def schedule_reservation(self, reservation: Reservation) -> None:
        ...
        for card in self.node.network_manager.get_timecards():
            if reservation in card.reservations:
                process = Process(
                    self, "add_memo_reservation_map", [card.memory_index, reservation]
                )
                event = Event(reservation.start_time, process)
                self.node.timeline.schedule(event)
                process = Process(
                    self, "remove_memo_reservation_map", [card.memory_index]
                )
