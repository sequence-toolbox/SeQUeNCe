...
@pytest.mark.parametrize(
    "psi,seeds", list(itertools.product(_single_inputs, _single_seeds))
)
def test_teleport_recreates_state(psi, seeds):
    ...
    assert verify_same_state_vector(out, psi), (
        f"teleported state {out} != original {psi}"
    )
...
_dual_inputs = [
    (_random_state(_rng_inputs), _random_state(_rng_inputs)) for _ in range(2)
]
_dual_seeds = [
    {
        "alice": int(_rng_inputs.integers(0, 2**31 - 1)),
...
def test_dual_teleport_recreates_states(psi_b, psi_c, seeds):
    ...
    assert check_bob and check_charlie, (
        f"Bob:{check_bob}, got={out_b}, correct={psi_b}. Charlie:{check_charlie}, got={out_c}, correct={psi_c}"
    )
