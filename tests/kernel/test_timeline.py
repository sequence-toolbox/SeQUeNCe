...
def _set_up_test(activation_method: str, stop_time=Timeline.STOP_TIME_INFINITE, number_of_dummys: int = 1, event_time: int = 10, event_priority=Event.PRIORITY_LOWEST):
    ...
    events = [Event(event_time, process, event_priority)
...
def test_run():
    ...
    timeline, [dummy], _ = _set_up_test(
        'operate', event_time=time, event_priority=priority)

    timeline.run()

    assert dummy.counter == 1
    assert timeline.now() == timeline.time == time and len(
        timeline.events) == _INITIAL_COUNT
...
def test_run_with_stop_time():
    ...
    time = random.randint(0, 20)
    priority = random.randint(0, 20)
    timeline, [dummy], events = _set_up_test(
        'operate', stop_time, event_time=time, event_priority=priority)

    timeline.run()

    assert dummy.counter == _INITIAL_COUNT
    assert timeline.now() == timeline.time < stop_time and len(
        timeline.events) == len(events)


def test_remove_event():
    timeline, dummys, events = _set_up_test(
        'operate', number_of_dummys=2, event_time=1)
