...
def _set_up_test(
    ...
    stop_time=inf,
    number_of_dummys: int = 1,
    event_time: int = 10,
    event_priority=inf,
) -> Timeline:
    timeline = Timeline(stop_time)
    dummys = [
        Dummy(f"{dummy_number}", timeline) for dummy_number in range(number_of_dummys)
    ]
...
def test_run():
    ...
    assert (
        timeline.now() == timeline.time == time
        and len(timeline.events) == _INITIAL_COUNT
    )
...
def test_run_with_stop_time():
    ...
    assert timeline.now() == timeline.time < stop_time and len(timeline.events) == len(
        events
    )
