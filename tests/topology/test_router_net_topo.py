...
from sequence.network_management.routing.routing_distributed import (
...
class TestGenerateConfig:
    ...
    def test_stop_time(self):
        config, *_ = nx_converter.generate_config(
            graphs.build_linear(2), cc_delay=1, stop_time=100
        )
    ...
    def test_template_references(self):
        ...
        assert all(
            n["template"]
            == ("router_template" if n["type"] == "QuantumRouter" else "bsm_template")
...
def test_router_net_topo_config():
    ...
    for node in topo.get_nodes_by_type(RouterNetTopo.QUANTUM_ROUTER):
        ...
        for memory in memory_array.memories:
            ...
            assert memory.decoherence_errors == pytest.approx(
                [1 / 3, 1 / 3, 1 / 3], abs=EPSILON
            )
