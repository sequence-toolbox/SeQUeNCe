...
def test_Transducer():
    ...
    class FakeUpConversionProtocol(UpConversionProtocol):
        def __init__(
            self, owner: FakeNode, name: str, tl: Timeline, transducer: Transducer
        ):
    ...
    transducer.up_conversion_protocol = FakeUpConversionProtocol(
        node, f"{node.name}.up_convertion_protocol", tl, transducer
    )
    transducer.down_conversion_protocol = FakeDownConversionProtocol(
        node, f"{node.name}.down_convertion_protocol", tl, transducer
    )
