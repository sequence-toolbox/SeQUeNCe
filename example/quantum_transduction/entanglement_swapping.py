...
class SenderNode(Node):
    ...
    def __init__(self, name, timeline):
        ...
        transducer = Transducer(
            name=self.transducer_name,
            owner=self,
            timeline=timeline,
            efficiency=EFFICIENCY_UP,
        )
        ...
        self.emitting_protocol = EmittingProtocol(
            self, name + ".emitting_protocol", timeline, transmon0, transducer
        )
        transducer.up_conversion_protocol = UpConversionProtocol(
            self, name + "up_conversion_protocol", timeline, transducer
        )
...
class EntangleNode(Node):
    ...
    def __init__(self, name, timeline, src_list: list[str]):
        ...
        detector = FockDetector(
            detector_name, timeline, efficiency=OPTICAL_DETECTOR_EFFICIENCY
        )
        self.add_component(detector)

        detector_name2 = name + ".detector2"
        detector2 = FockDetector(
            detector_name2, timeline, efficiency=OPTICAL_DETECTOR_EFFICIENCY
        )
        ...
        self.swapping_protocol = Swapping(
            self, name + ".swapping_protocol", timeline, fock_beam_splitter
        )
        self.measure_protocol = Measure(
            self, name + ".measure_protocol", timeline, fock_beam_splitter
        )
...
if __name__ == "__main__":
    ...
    qc1 = FockQuantumChannel(
        "qc.node1.node2", tl, attenuation=ATTENUATION, distance=DISTANCE
    )
    qc2 = FockQuantumChannel(
        "qc.node3.node2", tl, attenuation=ATTENUATION, distance=DISTANCE
    )
    ...
    for trial in range(NUM_TRIALS):
        ...
        detector_photon_counter_ideal = (
            node2.measure_protocol.detector_photon_counter_ideal
        )
        spd_ideal = node2.measure_protocol.spd_ideal
        detector_photon_counter_real = (
            node2.measure_protocol.detector_photon_counter_real
        )
    ...
    percentage_detector_counters_ideal = (
        detector_photon_counter_ideal / total_emitted_photons
    ) * 100
    print(
        f"Percentage of Entangled pairs generated (PNRD IDEAL): {percentage_detector_counters_ideal:.2f}%"
    )

    percentage_detector_counters_real = (
        detector_photon_counter_real / total_emitted_photons
    ) * 100
    print(
        f"Percentage of Entangled pairs detected by PNRD real: {percentage_detector_counters_real:.2f}%"
    )
    ...
    plt.plot(
        times, spd_ideals, "o-", color="darkblue", label="SPD Ideal", markersize=4
    )  # blu pieno
    plt.plot(
        times,
        spd_reals,
