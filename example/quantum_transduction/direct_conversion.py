...
class SenderNode(Node):
    ...
    def __init__(self, name, timeline):
        ...
        transmon = Transmon(
            ...
            photon_counter=0,
            efficiency=TRANSMON_EFFICIENCY,
        ...
        detector = FockDetector(
            detector_name,
            timeline,
            wavelength=MICROWAVE_WAVELENGTH,
        ...
        self.emitting_protocol = EmittingProtocol(
            self, name + ".emitting_protocol", timeline, transmon, transducer
        )
...
class ReceiverNode(Node):
    ...
    def __init__(self, name, timeline):
        ...
        transducer.down_conversion_protocol = DownConversionProtocol(
            self, name + "down_conversion_protocol", timeline, transducer
        )
...
if __name__ == "__main__":
    ...
    if (
        EFFICIENCY_UP >= 0
        and EFFICIENCY_UP <= 1
        and EFFICIENCY_DOWN >= 0
        and EFFICIENCY_DOWN <= 1
    ):
        pass
    else:
    ...
    print(f"--------------------")
    print(
        f"Direct Quantum Transduction Protocol starts, the qubit that we are going to convert is: {KET1}"
    )
    ...
    for trial in range(NUM_TRIALS):
        ...
        print(
            f"Number of photons converted at time {tl.time}: {transmon2.photon_counter}"
        )
    ...
    conversion_percentage = (
        (total_photons_successful / total_photons_to_be_converted) * 100
        if total_photons_to_be_converted > 0
        else 0
    )
    print(
        f"Conversion efficiency of DQT protocol with no-idealities of transmon: {conversion_percentage:.2f}%"
    )
...
fig, (ax1, ax2) = plt.subplots(
    2, 1, sharex=True, figsize=(12, 8), gridspec_kw={"height_ratios": [4, 1]}
)
...
(l1,) = ax1.plot(
    ...
    ideal_photons,
    "o-",
    label="Ideal Successfully Converted Photons",
    color="darkblue",
    markersize=3,
)
(l2,) = ax1.plot(
    ...
    converted_photons,
    "o-",
    label="Successfully Converted Photons",
    color="#FF00FF",
    markersize=3,
)
...
b3 = ax2.bar(
    ...
    label="Successful conversions",
    alpha=0.8,
    bottom=results_matrix[:, 0] + results_matrix[:, 1],
    width=PERIOD * 0.8,
    align="edge",
)
