...
class EntanglementSwappingA(EntanglementProtocol):
    ...
    def start(self) -> None:
        ...
        log.logger.info("{} middle protocol start with ends {}, {}".format(
            self.own.name, self.left_protocol_name,
        ...
        if random() < self.success_probability():
            ...
            meas_samp = self.own.generator.random()
...
class EntanglementSwappingB(EntanglementProtocol):
    ...
    def start(self) -> None:
        log.logger.info(
            self.own.name + " end protocol start with partner {}".format(
