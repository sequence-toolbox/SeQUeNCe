...
from numpy import outer, zeros, add, array_equal
...
from ..kernel.quantum_manager import KET_STATE_FORMALISM as KET
from ..kernel.quantum_manager import QuantumManagerDensity as DENSITY
...
def make_bsm(name, timeline, encoding_type='time_bin', phase_error=0,
             detectors=[]):
    ...
    def trigger(self, detector: Detector, info: Dict[str, Any]):
        ...
        if round((time - self.last_res[0]) / self.encoding_type["bin_separation"]) == 1:
            ...
            if detector_num == self.last_res[1]:
                info = {'entity': 'BSM', 'info_type': 'BSM_res', 'res': 0,
                        'time': time}
                self.notify(info)
            # Psi-
            else:
                info = {'entity': 'BSM', 'info_type': 'BSM_res', 'res': 1,
                        'time': time}
                self.notify(info)
