...
class SimpleManager:
    ...
    def create_protocol(self):
        if type(self.owner) is SwapNodeA:
            ...
            self.owner.protocols = [
                EntanglementSwappingB.create(
                    self.owner, "%s.ESB" % self.owner.name, memo
                )
            ]
...
class SwapNodeB(Node):
    ...
    def create_protocol(self):
        self.protocols = [
            EntanglementSwappingB.create(self, "%s.ESB" % self.name, self.memo)
        ]
...
def pair_protocol(node1, node2, node_mid):
    ...
    p1.set_others(
        ...
        node_mid.name,
        [
            node_mid.resource_manager.memo_names[0],
            node_mid.resource_manager.memo_names[1],
        ],
    )
    p2.set_others(
        pmid.name,
...
if __name__ == "__main__":
    ...
    for i in range(3):
        for j in range(3):
            if i != j:
                cc = ClassicalChannel(
                    "cc_%s_%s" % (nodes[i].name, nodes[j].name), tl, 1000, 1e9
                )
