...
def eg_rule_condition(memory_info: "MemoryInfo", manager: "MemoryManager", args):
    ...
    if memory_info.state == "RAW" and index_lower <= memory_info.index <= index_upper:
        return [memory_info]
    else:
        return []
...
def eg_req_func(protocols, args):
    ...
    for protocol in protocols:
        ...
        if (
            protocol.remote_node_name == remote_node
            and index_lower <= mem_arr.memories.index(protocol.memory) <= index_upper
        ):
            return protocol
...
def eg_rule_action1(memories_info: list["MemoryInfo"], args):
    ...
    protocol = EntanglementGenerationA.create(
        None, "EGA." + memory.name, mid_name, other_name, memory
    )
    req_args = {
...
def add_eg_rules(index: int, path: list[RouterNode], middles: list[BSMNode]):
    ...
    if index > 0:
        action_args = {
            "mid_name": middle_names[index - 1],
            "other_name": node_names[index - 1],
        }
        condition_args = {"index_lower": mem_range[0], "index_upper": mem_range[0] + 9}
    ...
    if index < (len(path) - 1):
        if index == 0:
            condition_args = {
                "index_lower": mem_range[0],
                "index_upper": mem_range[1] - 1,
            }
        else:
            condition_args = {
                "index_lower": mem_range[1] - 10,
                "index_upper": mem_range[1] - 1,
            }

        action_args = {
            "mid_name": middle_names[index],
            "other_name": node_names[index + 1],
            "node_name": node.name,
...
def ep_rule_condition1(memory_info: "MemoryInfo", manager: "MemoryManager", args):
    ...
    if (
        index_lower <= memory_info.index <= index_upper
...
def ep_rule_action1(memories_info: list["MemoryInfo"], args):
    ...
    req_args = {
        "remote1": memories_info[0].remote_memo,
    ...
    for node1 in node_list:
        for node2 in node_list:
            if node1.name != node2.name:
                cc = ClassicalChannel(
                    "cc_%s_%s" % (node1.name, node2.name), tl, 1e3, delay=cc_delay
                )
