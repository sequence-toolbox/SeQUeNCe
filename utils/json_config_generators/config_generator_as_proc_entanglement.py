...
def get_partition(graph, GROUP_NUM, flow_info):
    net_size = len(graph.nodes)

    def energy_func_cross_proc_entanglement(reverse_map_group, alpha):
        e = 0
        for src in flow_info:
            path = flow_info[src]
            flag = True
            for node in path:
                if reverse_map_group[src] != reverse_map_group[node]:
                    flag = False
                    break
            if not flag:
                e += alpha ** (len(path) - 2)
    ...
    class State():
        ...
        def get_energy(self):
            return energy_func_cross_proc_entanglement(self.reverse_map_group,
                                                       self.alpha)
...
if args.nodes:
    ...
    for src in selected_paths:
        ...
        flow_info[router_name_func(src)] = new_path

    groups = get_partition(graph, int(GROUP_NUM), flow_info)
