...
def ring_network(ring_size: int, lookahead: int, stop_time: int, rank: int,
    ...
    if rank != mpi_size - 1:
        ...
        for bsm_id in range(ring_size):
            ...
            if pre_node_name in tl.foreign_entities \
                    or post_node_name in tl.foreign_entities:
                if pre_node_name in tl.foreign_entities \
                        and post_node_name in tl.foreign_entities \
                        and tl.foreign_entities[pre_node_name] == \
                        tl.foreign_entities[post_node_name]:
                    tl.foreign_entities[node_name] = tl.foreign_entities[
                        pre_node_name]
                else:
                    tl.foreign_entities[node_name] = mpi_size
                continue
        ...
        for bsm_id in range(ring_size):
            ...
            pre_node_name = 'Node_%d' % ((bsm_id - 1) % ring_size)
            post_node_name = 'Node_%d' % bsm_id
            if tl.foreign_entities[pre_node_name] != tl.foreign_entities[
                post_node_name]:
                node = BSMNode(node_name, tl, [pre_node_name, post_node_name])
                node.set_seed(ring_size + bsm_id)
                bsm_nodes.append(node)

        for src in bsm_nodes:
            for dst_index in range(ring_size):
                dst_name = "Node_%d" % dst_index
                if dst_name != src.name:
                    cc = ClassicalChannel("cc_%s_%s" % (src.name, dst_name),
    ...
    # for node in routers+bsm_nodes:
    #     print(rank, node.name)
    # print("*** ", rank, )
    # for node in tl.foreign_entities:
    #     print("   ", node, tl.foreign_entities[node])
